...
def prepare_multi_scale_data(
    src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32
):
    ...
    split_train = splitbase_trainval(
        osp.join(src_path, "train"),
        dst_train_path,
        gap=gap,
        subsize=subsize,
    ...
    # split val data
    split_val = splitbase_trainval(
        osp.join(src_path, "val"),
        dst_val_path,
        gap=gap,
        subsize=subsize,
        num_process=num_process,
    ...
    split_test = splitbase_test(
        osp.join(src_path, "test/images"),
        dst_test_path,
        gap=gap,
        subsize=subsize,
        num_process=num_process,
    ...
    generate_json_labels(
        dst_trainval_path, osp.join(dst_trainval_path, "trainval.json")
    )
    generate_json_labels(
        dst_test_base_path, osp.join(dst_test_base_path, "test.json"), trainval=False
    )
...
if __name__ == "__main__":
    ...
    # multi scale
    prepare_multi_scale_data(
        "/mnt/SSD/lwt_workdir/data/dota_new/",
        "/data2/tiancai/datasets/DOTA1_0_ms/",
        scales=[0.5, 1.0, 1.5],
        gap=500,
    )
    print("done")
