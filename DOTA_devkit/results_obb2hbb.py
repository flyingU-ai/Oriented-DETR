...
def parse_args():
    ...
    parser.add_argument(
        "--srcpath",
        default=r"/OrientedRepPoints/tools/parse_pkl/evaluation_results/OBB_results/",
    )
    parser.add_argument(
        "--dstpath",
        default="/OrientedRepPoints/tools/parse_pkl/evaluation_results/HBB_results/",
        help="dota version",
    )
...
def OBB2HBB(srcpath, dstpath):
    ...
    for file in filenames:  # eg: /.../task1_plane.txt
        ...
        with open(file, "r") as f_in:
            with open(
                os.path.join(dstpath, "Task2_" + class_basename + ".txt"), "w"
            ) as f_out:
