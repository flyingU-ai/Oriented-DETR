...
def poly2csl(
    labels, num_cls_thata=360, radius=6.0, ignore_cls=[10, 12], use_gaussian=False
):
    """
    ...
    for poly, c in zip(polys, cls):
        ...
        if c in ignore_cls:
            angle = 0
        angle1 = gaussian_label_cpu(
            angle % 360, num_class=num_cls_thata, u=0, sig=radius
        )
        angle2 = gaussian_label_cpu(
            (angle + 90) % 360, num_class=num_cls_thata, u=0, sig=radius
        )
        angle3 = gaussian_label_cpu(
            (angle + 180) % 360, num_class=num_cls_thata, u=0, sig=radius
        )
        angle4 = gaussian_label_cpu(
            (angle + 270) % 360, num_class=num_cls_thata, u=0, sig=radius
        )
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            im = cv2.warpPerspective(
                im, M, dsize=(width, height), borderValue=(0, 0, 0)
            )
        else:  # affine
...
class CocoDetection(_CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if nl:
            rboxes, csl_labels = poly2csl(
                labels=labels[:, :], num_cls_thata=360, radius=2, use_gaussian=True
            )
            labels_mask = (
                (rboxes[:, 0] >= 0)
                & (rboxes[:, 0] < img.shape[1])
                & (rboxes[:, 1] >= 0)
                & (rboxes[:, 1] < img.shape[0])
                & (rboxes[:, 2] > 5)
                & (rboxes[:, 3] > 5)
            )
        ...
        if nl:
            ...
            s = (
                torch.tensor([self.img_size, self.img_size])
                .unsqueeze(dim=0)
                .unsqueeze(dim=0)
            )
            vecs = vecs / s
...
def build(image_set, args):
    ...
    PATHS = {
        "train": (
            root / "trainval2017",
            root / "annotations" / f"{mode}_trainval2017.json",
        ),
        "train_reg": (
            root / "trainval2017",
            root / "annotations" / f"{mode}_trainval2017.json",
