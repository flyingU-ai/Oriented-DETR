...
def poly2bbox(polys, angles):
    ...
    direction = torch.cat(
        [
            torch.sin(angles * torch.pi / 180).unsqueeze(0),
            -torch.cos(angles * torch.pi / 180).unsqueeze(0),
        ],
        dim=0,
    )
...
def detect(model, postprocessors, data_loader, base_ds, device, args, logger=None):
            ...
            for i in range(num_classes):
                ...
                for i in range(len(boxes_current_class)):
                    output_per_class[class_id].append(
                        [
                            current_image_id,
                            scores_cureent_class[i],
                            boxes_current_class[i],
                        ]
                    )
    ...
    for img_id, predict in output_per_image.items():
        ...
        img_path = os.path.join(
            args.coco_path, "test2017", img_filename
        )  # 你的图像目录
        # 读取和显示图像
    ...
    for class_id, predict_all in output_per_class.items():
        ...
        for predict in predict_all:
            ...
            with open(
                os.path.join(txtdir, "Task1_{}.txt".format(class_name)), "a"
            ) as f:
...
@torch.no_grad()
def evaluate(
    model, postprocessors, data_loader, base_ds, device, args=None, logger=None
):
    model.eval()
    class_names = base_ds.dataset["categories"]
    ...
    result_075 = compute_metric(
        gt, output_per_class, class_names, img_ids, ovthresh=0.75
    )
