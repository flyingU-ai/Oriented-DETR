...
class DeformableTransformer(nn.Module):
    ...
    def get_orient_proposals(self, output_proposals, polar_length, move_sum):

        ref_unsig = output_proposals[:, :, 0:2]
        new_ref_unsig = ref_unsig + move_sum
        new_ref_sig = new_ref_unsig.sigmoid()
        ref_sig = ref_unsig.sigmoid()
        l_unsig = output_proposals[:, :, 3:4]
        l_unsig = l_unsig.repeat(1, 1, 12)
        new_l_unsig = l_unsig + polar_length
        new_l_sig = new_l_unsig.sigmoid()
        angles = torch.linspace(0, 2 * math.pi, 12 + 1, device=output_proposals.device)[
            :-1
        ]
        base_vec = torch.stack([torch.sin(angles), -torch.cos(angles)], dim=-1)
        base_vec = base_vec.unsqueeze(dim=0).unsqueeze(dim=0)
        ref_sig = ref_sig.unsqueeze(dim=2)
        ref_sig = torch.cat([ref_sig for i in range(12)], dim=2)
        delta = new_l_sig.unsqueeze(dim=-1) * base_vec
        new_proposal = ref_sig + delta
        new_proposal = torch.cat([new_proposal, new_ref_sig.unsqueeze(-2)], dim=-2)
        new_proposal = inverse_sigmoid(new_proposal)
        return new_proposal
    ...
    def forward(self, srcs, masks, pos_embeds, attn_mask=None):
        """
        Input:
        ...
        polar_length = self.enc_out_bbox_embed(output_memory)
        center_move = self.enc_out_bbox_embed_sum(output_memory)
        enc_outputs_coord_unselected = self.get_orient_proposals(
            output_proposals, polar_length, center_move
        )
        ...
        refpoint_embed_undetach = torch.gather(
            ...
            1,
            topk_proposals.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 13, 2),
        )  # unsigmoid
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        """
        output = tgt.unsqueeze(dim=-2).repeat(1, 1, 13, 1)
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                delta_unsig_part = self.bbox_embed[layer_id](output[:, :, 0:12, :])
                outputs_unsig_part = (
                    delta_unsig_part + reference_before_sigmoid[:, :, 0:12, :]
                )
                delta_unsig_sum = self.bbox_embed_sum[layer_id](output[:, :, 12:13, :])
                outputs_unsig_sum = (
                    delta_unsig_sum + reference_before_sigmoid[:, :, 12:13, :]
                )
                outputs_unsig = torch.cat(
                    [outputs_unsig_part, outputs_unsig_sum], dim=-2
                )
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        assert sorted(module_seq) == ["ca", "ffn", "intra_sa", "sa"]
        # cross attention
        if use_deformable_box_attn:
        ...
        self.cross_attn_sum = MSDeformAttn(d_model, n_levels, n_heads, n_points)
        self.dropout1_sum = nn.Dropout(dropout)
        self.norm1_sum = nn.LayerNorm(d_model)

        # self attention
        self.self_attn = nn.MultiheadAttention(d_model, 8, dropout=dropout)
        # self.tgt_squeeze = nn.Linear(d_model, d_model // 8)
        # self.tgt_unsqueeze = nn.Linear(d_model // 8, d_model)
        # self.tgt_query_pos_squeeze = nn.Linear(d_model, d_model // 8)

        self.dropout2 = nn.Dropout(dropout)
        self.norm2 = nn.LayerNorm(d_model)

        # intra sa
        self.intra_self_attn = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_inta = nn.Dropout(dropout)
        self.norm_intra = nn.LayerNorm(d_model)
        self.intra_pos = nn.Parameter(torch.Tensor(13, 256))
        ...
        self.linear1_sum = nn.Linear(d_model, d_ffn)
        self.activation_sum = _get_activation_fn(activation, d_model=d_ffn, batch_dim=1)
        self.dropout3_sum = nn.Dropout(dropout)
        self.linear2_sum = nn.Linear(d_ffn, d_model)
        self.dropout4_sum = nn.Dropout(dropout)
        self.norm3_sum = nn.LayerNorm(d_model)
        #
    ...
    def forward_ffn_sum(self, tgt):
        tgt2 = self.linear2_sum(
            self.dropout3_sum(self.activation_sum(self.linear1_sum(tgt)))
        )
        tgt = tgt + self.dropout4_sum(tgt2)
        tgt = self.norm3_sum(tgt)
        return tgt
    ...
    def forward_sa(
        ...
        if self.self_attn is not None:
            if self.decoder_sa_type == "sa":
                tgt = tgt[:, :, 12, :]
                tgt_query_pos = tgt_query_pos[:, :, 12, :]
    ...
    def forward_ca_sum(
        ...
        # cross attention
        tgt_query_pos = tgt_query_pos[:, :, 12, :]
        tgt_reference_points = tgt_reference_points[:, :, :, 12, :]
        ...
        tgt = tgt + self.dropout1_sum(tgt2)
        tgt = self.norm1_sum(tgt)
    ...
    def forward_ca(
        ...
        tgt_query_pos = tgt_query_pos[:, :, 0:12, :]
        tgt_reference_points = tgt_reference_points[:, :, :, 0:12, :]
        n_q, bs, n_p, dim = tgt.shape
        tgt = tgt.transpose(1, 2).contiguous().view(-1, bs, dim)
        tgt_query_pos = tgt_query_pos.transpose(1, 2).contiguous().view(-1, bs, dim)
        tgt_reference_points = (
            tgt_reference_points.permute(0, 3, 1, 2, 4).contiguous().view(-1, bs, 4, 2)
        )
        ...
        tgt = self.norm1(tgt)
        tgt = tgt.view(n_q, n_p, bs, dim).transpose(1, 2)
        return tgt

    def forward_intra_sa(
        self,
        # for tgt
        tgt: Optional[Tensor],  # nq, bs, d_model
        tgt_query_pos: Optional[Tensor] = None,  # pos for query. MLP(Sine(pos))
        ...
        # self attention
        if self.intra_self_attn is not None:
            # tgt = tgt[:, :, 0:12, :]
            intra_pos = self.intra_pos.unsqueeze(0).unsqueeze(0)
            tgt = tgt + intra_pos
            # tgt_query_pos = tgt_query_pos[:, :, 0:12, :]
            n_q, bs, n_p, dim = tgt.shape
            tgt = tgt.view(-1, n_p, dim).transpose(0, 1)
            tgt_query_pos = tgt_query_pos.view(-1, n_p, dim).transpose(0, 1)

            q = k = self.with_pos_embed(tgt, tgt_query_pos)
            tgt2 = self.intra_self_attn(q, k, tgt, attn_mask=None)[0]
            tgt = tgt + self.dropout_inta(tgt2)
            tgt = self.norm_intra(tgt)
            tgt = tgt.transpose(0, 1).view(n_q, bs, n_p, dim)
    ...
    def forward(
        ...
        for funcname in self.module_seq:
            if funcname == "ffn":
                part_tgt = self.forward_ffn(part_tgt)
                sum_tgt = self.forward_ffn_sum(sum_tgt)
                part_tgt = self.norm_p(part_tgt)
                sum_tgt = self.norm_s(sum_tgt)
                tgt = torch.cat([part_tgt, sum_tgt.unsqueeze(-2)], dim=-2)
            elif funcname == "ca":
                part_tgt = self.forward_ca(
                    part_tgt,
                    tgt_query_pos,
                ...
                part_tgt = tgt[:, :, 0:12, :]
            elif funcname == "intra_sa":
                tgt = self.forward_intra_sa(
                    tgt,
                    tgt_query_pos,
                    tgt_query_sine_embed,
                    tgt_key_padding_mask,
