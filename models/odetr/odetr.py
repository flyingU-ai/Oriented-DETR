...
class DINO(nn.Module):
    ...
    def __init__(
        ...
        if dec_pred_bbox_embed_share:
            ...
            box_embed_sum_layerlist = [
                _bbox_embed_sum for i in range(transformer.num_decoder_layers)
            ]
        else:
            box_embed_layerlist = [
        ...
        if dec_pred_class_embed_share:
            ...
            class_squeeze_layerlist = [
                _class_squeeze for i in range(transformer.num_decoder_layers)
            ]
        else:
            class_embed_layerlist = [
            ...
            class_squeeze_layerlist = [
                copy.deepcopy(_class_squeeze)
                for i in range(transformer.num_decoder_layers)
            ]
        if dec_pred_angle_embed_share:
            angle_embed_layerlist = [
                _angle_embed for i in range(transformer.num_decoder_layers)
            ]
            angle_squeeze_layerlist = [
                _angle_squeeze for i in range(transformer.num_decoder_layers)
            ]
        else:
            angle_embed_layerlist = [
                copy.deepcopy(_angle_embed)
                for i in range(transformer.num_decoder_layers)
            ]
            angle_squeeze_layerlist = [
                copy.deepcopy(_angle_squeeze)
                for i in range(transformer.num_decoder_layers)
            ]
        ...
        if two_stage_bbox_embed_share:
            assert (
                ...
                and dec_pred_bbox_embed_share
                and dec_pred_angle_embed_share
            )
    ...
    def forward(self, samples: NestedTensor, targets: List = None):
        ...
        for dec_lid, (
            layer_ref_sig,
            layer_bbox_embed,
            layer_bbox_embed_sum,
            layer_hs,
        ) in enumerate(zip(reference[:-1], self.bbox_embed, self.bbox_embed_sum, hs)):
            layer_delta_unsig_part = layer_bbox_embed(layer_hs[:, :, 0:12, :])
            layer_outputs_unsig_part = layer_delta_unsig_part + inverse_sigmoid(
                layer_ref_sig[:, :, 0:12, :]
            )
            layer_delta_unsig_sum = layer_bbox_embed_sum(layer_hs[:, :, 12:13, :])
            layer_outputs_unsig_sum = layer_delta_unsig_sum + inverse_sigmoid(
                layer_ref_sig[:, :, 12:13, :]
            )
            layer_outputs_unsig = torch.cat(
                [layer_outputs_unsig_part, layer_outputs_unsig_sum], dim=-2
            )
            layer_outputs_unsig = layer_outputs_unsig.sigmoid()
            outputs_coord_list.append(layer_outputs_unsig)
        outputs_coord_list = torch.stack(outputs_coord_list)
        outputs_angle = torch.stack(
            [
                layer_angle_embed(layer_angle_squeeze(layer_hs).flatten(-2))
                for layer_angle_embed, layer_angle_squeeze, layer_hs in zip(
                    self.angle_embed, self.angle_squeeze, hs
                )
            ]
        )
        outputs_class = torch.stack(
            [
                layer_cls_embed(layer_cls_squeeze(layer_hs).flatten(-2))
                for layer_cls_embed, layer_cls_squeeze, layer_hs in zip(
                    self.class_embed, self.class_squeeze, hs
                )
            ]
        )
        ...
        if self.aux_loss:
            out["aux_outputs"] = self._set_aux_loss(
                outputs_class, outputs_coord_list, outputs_angle
            )
        # for encoder output
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_angle):
        ...
        return [
            ...
            for a, b, c in zip(
                outputs_class[:-1], outputs_coord[:-1], outputs_angle[:-1]
            )
        ]
...
class SetCriterion(nn.Module):
    ...
    def loss_angles(self, outputs, targets, indices, num_boxes):
        ...
        idx = self._get_src_permutation_idx(indices)
        target_angles_o = torch.cat(
            [t["angles"][J] for t, (_, J) in zip(targets, indices)]
        )
        querys_for_pre = src_logits[idx]
        angle_ce_loss = sigmoid_focal_loss(
            querys_for_pre, target_angles_o, num_boxes, alpha=0.8, gamma=2
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_boxes = torch.cat(
            [t["vecs"][i] for t, (_, i) in zip(targets, indices)], dim=0
        )
        ...
        loss_center = F.l1_loss(
            src_center.to(dtype=torch.float64), tgt_cen, reduction="none"
        )
        losses = {}
        losses["loss_bbox"] = (
            loss_bbox.sum() / num_boxes + loss_center.sum() / num_boxes
        )
...
class PostProcess(nn.Module):
    ...
    def rbox2poly(self, obboxes):
        ...
        if isinstance(obboxes, torch.Tensor):
            ...
            return torch.cat((point1, point2, point3, point4), dim=-1).reshape(
                *order, 8
            )
        else:
    ...
    def forward(self, outputs, target_sizes, inf_conf=0.005):
        ...
        topk_values, topk_indexes = torch.topk(
            prob.view(out_logits.shape[0], -1), 400, dim=1
        )
        ...
        polys = torch.gather(
            out_bbox, 1, topk_boxes.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 13, 2)
        )
        ...
        theta_pred = torch.cat(
            [theta_pred1, theta_pred2, theta_pred3, theta_pred4], dim=-1
        )
