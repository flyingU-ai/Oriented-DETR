...
def generate_base_vec(n_sample=12):

    angles = torch.linspace(0, 2 * math.pi, n_sample + 1)[:-1]
    base_vec = torch.stack([torch.sin(angles), torch.cos_(angles)], dim=-1)
...
def gen_encoder_output_proposals(
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        ...
        # base_vec = generate_base_vec(n_sample= n_sample)
